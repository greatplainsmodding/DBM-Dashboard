module.exports = {
	...
	mod: function (DBM) {
		...
		const express = DBM.require('express'),
			...
			actions = fs.readdirSync(`./extensions/dbm_dashboard_extension/actions/`).filter(file => file.endsWith(".js"));
		...
		for (let file of actions) {
			let pull = require(`../extensions/dbm_dashboard_extension/actions/${file}`);
		...
		DBM.Bot.onReady = function () {
			...
			app.get('/dashboard/@me/servers/:guildID', checkAuth, function (req, res) {
				...
				res.redirect(`https://discordapp.com/oauth2/authorize?client_id=${DBM.Bot.bot.user.id}&scope=bot&permissions=2146958591`);
			});

			app.get('/dashboard/admin', checkAuthOwner, function (req, res) {
				let config = dashboardConfig();
				let dashboardMods = [];
				app.actions.forEach(data => dashboardMods.push(data));
